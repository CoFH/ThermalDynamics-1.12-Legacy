...
public class TDCrafting {
    ...
    public static boolean useHardendedGlass = true;
    public static boolean useTransposerRecipes = true;

    public static void loadRecipes() {
        useHardendedGlass = ThermalDynamics.config.get("Recipes", "useHardendGlass", true);
        useTransposerRecipes = ThermalDynamics.config.get("Recipes", "useTransposerRecipes", true);
        String glassHardened = useHardendedGlass ? "blockGlassHardened" : "blockGlass";
        ...
        GameRegistry.addRecipe(new ShapedOreRecipe(TDDucts.energySuperCondEmpty.itemStack, "IGI", "GEG", "IGI", 'I', "ingotElectrum", 'G',
                glassHardened, 'E', TDDucts.energyReinforced.itemStack));
        ...
        addTransposerFill(800, TDDucts.energyReinforcedEmpty.itemStack, TDDucts.energyReinforced.itemStack, new FluidStack(
                TFFluids.fluidRedstone, 200), false);
        addTransposerFill(800, TDDucts.energyResonantEmpty.itemStack, TDDucts.energyResonant.itemStack, new FluidStack(
                TFFluids.fluidRedstone, 200), false);
        addTransposerFill(4000, TDDucts.energySuperCondEmpty.itemStack, TDDucts.energySuperCond.itemStack, new FluidStack(
        ...
        addTransposerFill(800, TDDucts.itemBasic.itemStack, TDDucts.itemFast.itemStack, new FluidStack(TFFluids.fluidGlowstone, 200),
                false);
        ...
        int hardGlassLevel = useHardendedGlass ? 2 : 5; //level to start using hardened glass

        for (int i = 0; i < materials.length; i++) {
            GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(ThermalDynamics.itemServo, 2, i), "iGi", "IRI", 'R', "dustRedstone", 'G', i < hardGlassLevel ? "blockGlass" : "blockGlassHardened",
                    'I', "ingot" + materials[i], 'i', "nuggetIron"));

            GameRegistry.addRecipe(new ShapedOreRecipe(new ItemStack(ThermalDynamics.itemFilter, 2, i), "iGi", "IRI", 'R', Items.paper, 'G', i < hardGlassLevel ? "blockGlass" : "blockGlassHardened", 'I',
            ...
            if (i > 0) {
                for (Item item : new Item[]{ThermalDynamics.itemFilter, ThermalDynamics.itemServo, ThermalDynamics.itemRetriever}) {
                    if (i < hardGlassLevel) {
                        GameRegistry.addRecipe(addInputMetaRange(new ShapelessOreRecipe(new ItemStack(item, 1, i), "ingot" + materials[i]),
                                new ItemStack(item, 1), 0, i - 1));
                    } else {
                        if (i > hardGlassLevel) {
                            GameRegistry.addRecipe(addInputMetaRange(new ShapelessOreRecipe(new ItemStack(item, 1, i), "ingot" + materials[i]),
                                    new ItemStack(item, 1), hardGlassLevel, i - 1));
                        }

                        GameRegistry.addRecipe(
                                addInputMetaRange(addInputMetaRange(
                                        new ShapelessOreRecipe(new ItemStack(item, 2, i),
                                                "blockGlassHardened",
                                                "ingot" + materials[i],
                                                "ingot" + materials[i]
                                        ), new ItemStack(item, 1), 0, hardGlassLevel - 1
                                ), new ItemStack(item, 1), 0, hardGlassLevel - 1)
                        );
                    }
                }
        ...
        for (DuctItem duct : new DuctItem[]{
                TDDucts.itemBasic,
                TDDucts.itemBasicOpaque,
                TDDucts.itemEnder,
                TDDucts.itemEnderOpaque,
                TDDucts.itemFast,
