...
public class CoverHoleQuad {

	public static final ITransformer[] hollowDuct = CoverHoleQuad.hollowCover(0.3125f);
	public static final ITransformer[] hollowDuctLarge = CoverHoleQuad.hollowCover(0.28125f);
	public static final ITransformer[] hollowDuctTile = CoverHoleQuad.hollowCover(0.25f);

	public static final ITransformer[] hollowDuctCryo = CoverHoleQuad.octaCover(0.5F - 0.375F, 0.1812F);
	public static final ITransformer[] hollowDuctTransport = CoverHoleQuad.octaCover(0.5F * (1 - TDProps.largeInnerModelScaling), 0.1812F);


	Vertex8[] verts;

	public CoverHoleQuad(Vertex8[] verts) {
		this.verts = verts;
	}

	public static void holify(int startIndex, int x, int y, int z, int side, ITransformer[] transformers) {
		List<CoverHoleQuad> tessQuads = loadFromTessellator(startIndex, true);
		for (CoverHoleQuad tessQuad : tessQuads) {
			tessQuad.sliceStretchDraw(x, y, z, side, transformers);
		}
	}

	public static List<CoverHoleQuad> loadFromTessellator(int startIndex, boolean pop) {
		...
		List<CoverHoleQuad> list = new ArrayList<CoverHoleQuad>((endIndex - startIndex) >> 2);
		...
		for (int k = startIndex; k < endIndex; k += 32) {
			...
			list.add(new CoverHoleQuad(verts));
		}
	...
	public void sliceStretchDraw(int x, int y, int z, int side, ITransformer[] transformers) {
		...
		for (ITransformer transformer : transformers) {
			CoverHoleQuad slice = slice(x, y, z, side, transformer, uvTransform);
			if (slice.notEmpty())
				slice.draw();
		}
	}
	...
	public CoverHoleQuad slice(int x, int y, int z, int side, ITransformer transformer, float[][] uvTransform) {
		...
		return new CoverHoleQuad(v);
	}

	public void draw() {
		for (Vertex8 vert : verts) {
			vert.draw();
		}
	}
	...
	public static class Vertex8 implements Copyable<Vertex8> {
		...
		public void draw() {
			...
			tess.addVertex(this.x, this.y, this.z); // to grow the buffer if needed
		...
		public float[] buildTex() {
			return new float[]{
					...
					(brightness), (brightness >> 4) & 0x1fffff
			};
		}
		...
		public void reloadTex(float[] tex) {
			u = tex[0];
			v = tex[1];
			color = ((int) MathHelper.clampF(tex[2], 0, 255) << 24) | ((int) MathHelper.clampF(tex[3], 0, 255) << 16) | ((int) MathHelper.clampF(tex[4], 0, 255) << 8) | ((int) MathHelper.clampF(tex[5], 0, 255));
