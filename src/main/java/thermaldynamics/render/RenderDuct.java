...
public class RenderDuct implements ISimpleBlockRenderingHandler, IItemRenderer {
    ...
    public boolean renderFrame(boolean invRender, int renderType, int[] connection, double x, double y, double z) {
        ...
        int c = 0;
        Ducts ductType = Ducts.values()[renderType];
        for (int s = 0; s < 6; s++) {
        ...
        if (ductType.overDuctsType == 1) {
            renderSideTubes(0, connection, x - 0.5, y - 0.5, z - 0.5, sideDucts);
        }

        if (ductType.overDuctsType == 2 && ductType.iconOverDuctTexture != null) {
            c = 0;
            for (int s = 0; s < 6; s++) {
                if (BlockDuct.ConnectionTypes.values()[connection[s]].renderDuct() && connection[s] != BlockDuct.ConnectionTypes.STRUCTURE.ordinal()) {
                    ...
                    if (invRender || connection[s] != BlockDuct.ConnectionTypes.DUCT.ordinal()) {
                        ...
                        modelLargeDucts2[70 + s].render(x, y, z, RenderUtils.getIconTransformation(ductType.iconOverDuctTexture));
                    }
                }
            }

            if (modelLargeDucts1[c].verts.length != 0) {
                modelLargeDucts1[c].render(x, y, z, RenderUtils.getIconTransformation(ductType.iconOverDuctTexture));
            }
        }
    ...
    public boolean renderSideTubes(int pass, int[] connections, double x, double y, double z, IIcon icon) {
        ...
        int c = 0;
        for (int i = 0; i < 6; i++)
            if (BlockDuct.ConnectionTypes.values()[connections[i]].renderDuct() &&
                    connections[i] != BlockDuct.ConnectionTypes.CLEANDUCT.ordinal()) {
                c = c | (1 << i);
    ...
    public boolean renderWorldExtra(boolean invRender, TileMultiBlock tile, int renderType, int[] connection, double x, double y, double z) {
        ...
        if (ductType.overDuctsType == 1 && ductType.iconOverDuctInternalTexture != null) {
            flag = renderSideTubes(1, connection, x, y, z, ductType.iconOverDuctInternalTexture) || flag;
        }

        if (ductType.overDuctsType == 2 && ductType.iconOverDuctInternalTexture != null) {
            int c = 0;
            for (int s = 0; s < 6; s++) {
                if (BlockDuct.ConnectionTypes.values()[connection[s]].renderDuct() && connection[s] != BlockDuct.ConnectionTypes.STRUCTURE.ordinal()) {
                    c = c | (1 << s);
                    ...
                    if (invRender || connection[s] != BlockDuct.ConnectionTypes.DUCT.ordinal()) {
                        modelLargeDucts2[70 + s].render(x + 0.5, y + 0.5, z + 0.5,
                                RenderUtils.getIconTransformation(ductType.iconOverDuctInternalTexture));
                    }
                }
            }
            ...
            if (modelLargeDucts2[c].verts.length != 0) {
                modelLargeDucts2[c]
                        .render(x + 0.5, y + 0.5, z + 0.5, RenderUtils.getIconTransformation(ductType.iconOverDuctInternalTexture));
                flag = true;
            }
        }
